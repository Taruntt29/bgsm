...
const LeaveStatus = () => {
  ...
  console.log("GetLeaveStatusData", GetLeaveStatusData);
  ...
  const approveBtn = async (id) => {
    ...
    const confirmBox = window.confirm("Do you really want to Approve?");
    if (confirmBox === true) {
      await jwtAxios
        .post("http://3.111.162.16:8762/api/leave/leave-approve", payload)
        .then(() => {
          getLeaveStatusAction(dispatch);
        })
        .then(() => {
          toast
            .success("Approved Leave", {
              position: "top-right",
              autoClose: 4000,
